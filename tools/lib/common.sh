#!/usr/bin/env bash
# Shared helpers for feedstock tools (tebako-packages/fontist).
# Sourced by tools/publish.
...
# on ubuntu-24.04). The asset name carries the version, so the digest
# pinned in the recipe (signing.tool.sha256) is the trust anchor.
download_signer() {
